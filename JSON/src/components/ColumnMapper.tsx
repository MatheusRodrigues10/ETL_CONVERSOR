...
export const ColumnMapper = ({
  ...
  const handleNovasVariacoes = useCallback((
    ...
    if (novasVariacoes.length === 0) {
      // Se não há novas variações, apenas atualizar o campo novas_variacoes
      const existingIndex = mappings.findIndex(
        (m) => m.gabaritoColumn === gabaritoColumn && m.sourceFile === sourceFile
      );
    ...
    if (existingIndex >= 0) {
      const existingMapping = mappings[existingIndex];
      if (Array.isArray(existingMapping.sourceColumn)) {
        existingVariations = [...existingMapping.sourceColumn];
      } else if (
        existingMapping.sourceColumn &&
        existingMapping.sourceColumn !== "__EMPTY__"
      ) {
    ...
    // Combinar: adicionar apenas as que não existem
    const combinedVariations = [...existingVariations];
    novasVariacoes.forEach((nova) => {
      if (!combinedVariations.includes(nova)) {
        combinedVariations.push(nova);
      }
    });
    ...
    const newMapping: ColumnMapping = {
      gabaritoColumn,
      sourceColumn: combinedVariations.length > 0 ? combinedVariations : "__EMPTY__",
      sourceFile,
      novas_variacoes: novasVariacoesString.trim(),
    };
